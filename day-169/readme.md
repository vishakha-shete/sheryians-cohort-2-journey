# day-169 i give you task to read documentation and implement it 
...
- notification service in which we are sending email
...
- we are having the  this pods for running user projects
- this pods controlled by sandbox service  from creation till deletion 
- we are deploying our sandbox service on kuber cluster 
- the kubernatives cluster missing one thing called 
- needed to install the ingress controller bydefault it is not present in our system we have to install it 
- for installing ingress controller after installing this we can apply a rules 
- we have to run this command fro installing it 
- kubectl apply -f \
  https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.12.1/deploy/static/provider/cloud/deploy.yaml
  - if u not done this it gives error so need to install ingress controller first 
  - then create a image 
  - controller and deployement , service , ingress  
  - then now our sandbox services server  spins 
- the sandbox server is spins but dont write code how the pod is connected to the sandbox 
- we created a ingress called codespace-ingress 
- it works for if request came we created one another service called sandbox-service codespace-ingress redirect the traffic on the sandbox-service this servise sends a traffic on perticular sandbox-service 
- each pod created by user  has its own ingress and service